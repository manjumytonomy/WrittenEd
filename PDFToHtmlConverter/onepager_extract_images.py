...
'''This piece of code ensures that the new image position is accounted for and not repeated, also makes sure to remove the last image of every page'''
...
def calculate_width_height(bbox):
    x0, y0, x1, y1 = bbox  # Extract the coordinates from the bounding box
    width = x1 - x0        # Calculate the width as x1 - x0
    height = y1 - y0       # Calculate the height as y1 - y0
    return width, height   # Return the width and height


def is_qr_code(image_info):
    """
    Check if the image is a QR code based on its dimensions (width and height).
    """
    bbox = image_info["bbox"]
    width, height = calculate_width_height(bbox)
    
    # QR code dimensions (width and height)
    qr_width = 72.0
    qr_height = 56.0
    
    ######################################## Fix for WAM-91 Starts ####################################################
    # Allow for a tolerance of upto around 20 between the width and height of image 1 in each page (to keep things generic)
    # This is to try to avoid issues with logo by differentiating between logo and QR code
    tolerance = 20.0
    ######################################## Fix for WAM-91 Ends ####################################################
    
    return abs(width - qr_width) <= tolerance and abs(height - qr_height) <= tolerance
    

def process_images(images_info):
    print("PRINTING OUT ALL THE IMAGE DIMENSIONS")

    # Iterate over a copy of the list
    for image in images_info[:]:
        image_path, bbox, is_annot, alignment = image

        # Create an image_info dictionary for the bbox to pass to is_qr_code function
        image_info = {"bbox": bbox}
        
        # Check if the image is a QR code
        if is_qr_code(image_info) and 'img1' in image_path:
            print(f"QR code detected and removed: {image_path}")
            images_info.remove(image)  # Remove the image from the original list
...
def one_pager_extract_images_and_coordinates(pdf_path, page_number, scale):
    ...
    try:
        ...
        # Adjusted the scale factor to account for the difference in logo image sizing from Inova Logo 
        logo_scaled_coords = (0.400001525878906, 0.949974060058594, 145.39999389648438, 40.3499755859375)
        ...
        if configCustomer[f'{customer}'].getboolean('embed_customer_logo'):
            ...
            # Find and remove the last image on the page (largest y-coordinate)
            try:
                # Find the image with the largest y-coordinate (furthest down the page)
                max_y_image = max(images_info, key=lambda item: item[1][1])

                # Remove the last image from images_info
                images_info.remove(max_y_image)
                logging.info(f"Removed the last image with filename: {max_y_image[0]}")

            except ValueError as e:
                logging.error(f"Error finding or removing the last image: {e}")
                print(f"Error finding or removing the last image: {e}")

        print("PRINTING OUT ALL THE IMAGE DIMENSIONS")
        for image in images_info:
            print(image)

        process_images(images_info)
